...
text_cases = [
    ("O produto e bom. Excelente! Ele realmente supera as expectativas. Recomendo demais.", "positivo"),
    ("O produto dentro do esperado.", "neutro"),
    ("O produto e ruim.", "negativo"),
    ("Nao tive problemas, e tambem nao me surpreendi.", "neutro")
]
...
def test_model_prediction_lablels():
    ...
    assert pred in ['positivo','neutro','negativo'], f"Vectorized output should have one row for the sample text {pred}."
...
def test_data_validation():
    ...
    assert df['label'].isin(["positivo","neutro","negativo"]).all(), "All text entries should be non-null."
...
def test_sentimento_classificacao():
    ...
    for text, expected in text_cases:
        ...
        assert pred == expected, f"Expected {expected} but got {pred} for text: {text}"